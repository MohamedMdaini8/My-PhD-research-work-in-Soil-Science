...
p1 <- ggplot(df, aes(x = Csol, y = TN1)) +
  ...
  stat_cor(method = "spearman", label.x = min(df$Csol), label.y = max(df$TN1), size = 4) +
  labs(title = "A", x = "Soluble C (mg/kg)", y = "TN in >2 mm") +
  theme_minimal()

p2 <- ggplot(df, aes(x = Csol, y = TN2)) +
  geom_point(color = "#d95f02", size = 3, alpha = 0.8) +
  geom_smooth(method = "lm", se = TRUE, color = "#d95f02", fill = "#d95f02", alpha = 0.2) +
  stat_cor(method = "spearman", label.x = min(df$Csol), label.y = max(df$TN2), size = 4) +
  labs(title = "B", x = "Soluble C (mg/kg)", y = "TN in 0.25-2 mm") +
  theme_minimal()

p3 <- ggplot(df, aes(x = Csol, y = TN3)) +
  geom_point(color = "#7570b3", size = 3, alpha = 0.8) +
  geom_smooth(method = "lm", se = TRUE, color = "#7570b3", fill = "#7570b3", alpha = 0.2) +
  stat_cor(method = "spearman", label.x = min(df$Csol), label.y = max(df$TN3), size = 4) +
  labs(title = "C", x = "Soluble C (mg/kg)", y = "TN in 0.053-0.25 mm") +
  theme_minimal()

p4 <- ggplot(df, aes(x = Csol, y = TN4)) +
  geom_point(color = "#e7298a", size = 3, alpha = 0.8) +
  geom_smooth(method = "lm", se = TRUE, color = "#e7298a", fill = "#e7298a", alpha = 0.2) +
  stat_cor(method = "spearman", label.x = min(df$Csol), label.y = max(df$TN4), size = 4) +
